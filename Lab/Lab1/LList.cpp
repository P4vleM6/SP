...
{
	if (isEmpty()) return;                // prazna lista
	if (head->info == el) return;         // element je već na glavi

	LListNode* beforePrev = NULL;
	LListNode* prev = head;
	LListNode* curr = head->next;

	// tražimo prvo pojavljivanje el
	...
	if (curr == NULL) return;          // element ne postoji

	// curr je element koji pomeramo, prev je njegov prethodnik
	prev->next = curr->next;
	curr->next = prev;

	if (beforePrev == NULL) {
		// prev je bio head, pa curr ide na glavu
		head = curr;
	}
	else {
		// povezujemo beforePrev sa curr
